...
public class LauncherUpdateHandler : ILauncherUpdateHandler
{
    public static async Task<IResult> UploadLauncherVersion(HttpContext context, IGmlManager gmlManager)
    {
        ...
        if (string.IsNullOrEmpty(buildName))
            return Results.BadRequest(ResponseMessage.Create("Не удалось определить версию сборки",
        ...
        catch (Exception exception)
        {
            ...
            return Results.BadRequest(ResponseMessage.Create($"Не удалось обновить файл лаунчера: {exception.Message}",
    ...
    {
        return Results.Ok(ResponseMessage.Create(gmlManager.LauncherInfo.ActualLauncherVersion, string.Empty,
            HttpStatusCode.OK));
    }
