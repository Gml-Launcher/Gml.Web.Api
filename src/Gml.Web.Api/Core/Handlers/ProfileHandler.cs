...
public class ProfileHandler : IProfileHandler
{
        ...
        ProfileUpdateDto updateDto)
    {
        ...
        if (!result.IsValid)
        {
            return Results.BadRequest(ResponseMessage.Create(result.Errors, "Ошибка валидации", HttpStatusCode.BadRequest));
        }
        ...
        return Results.Ok(ResponseMessage.Create(mapper.Map<ProfileReadDto>(profile), "Профиль успешно обновлен", HttpStatusCode.OK));
    }
        ...
        string profileName)
    {
        ...
        if (profile is null)
            return Results.NotFound(ResponseMessage.Create($"Профиль \"{profileName}\" не найден", HttpStatusCode.NotFound));
