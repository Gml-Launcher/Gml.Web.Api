...
public class ProfileHub : BaseHub
{
    ...
    private async void SendProgress(string name, string profileName, double percentage)
    {
        try
        {
            ...
            if (double.IsPositiveInfinity(percentageValue) || double.IsNegativeInfinity(percentageValue)) return;

            if (double.IsNaN(percentageValue) || double.IsNaN(percentageValue)) return;
